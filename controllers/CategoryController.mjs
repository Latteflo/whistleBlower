...
export const getCategoryById = async (req, res) => {
  try {
    ...
    res.status(400).json({ success: false, message: 'Error retrieving category', error });
  }
};
...
export const getReportsByCategoryId= async (req, res) => {
  try {
    ...
    if (reports.length === 0) {
      return res.status(404).json({ success: false, message: 'Reports not found for this category' });
    }
    res.status(200).json({success: true,  data: reports });
  } catch (error) {
    res.status(500).json({success: false,  message: 'Error retrieving reports by category', error });
  }
};
...
export const updateCategory = async (req, res) => {
  ...
  if (!name || !id) {
    return res.status(400).json({success: false,  message: 'Name and ID fields are required' });
  }
